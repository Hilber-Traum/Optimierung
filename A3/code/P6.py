...
def euler_ohne_for_schleifen(N):
    ...
    leibniz_pi = (6*np.sum(leibniz_arr))**(1/2)
...
for i in range(len(N)):
    ...
    approx_euler.append(value)

    [value, time] = leibniz_ohne_for_schleifen(N[i])
    approx_leibniz.append(value)

#Plotte die Approximationen
plt.plot(N, approx_euler)
plt.plot(N, approx_leibniz)
plt.title("Approximationen von Euler und Leibniz für Pi")
plt.xlabel("Anzahl Summanden")
plt.ylabel("Approximation")
plt.show()

print("Der Plot zeigt, dass beide Varianten relativ langsam sind um pi zu berechnen, die Variante von Euler jedoch eine höhere Genauigkeit hat.")
